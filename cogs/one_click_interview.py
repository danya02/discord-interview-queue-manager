...
class OneClickInterview(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        logging.info("One-click interview cog initialized!")
        self.chan = None
    ...
    async def reset_everything(self):
        ...
        if not self.chan:
            logging.error("Action channel was not in cache, fetching it now!")
            self.chan = await self.bot.fetch_channel(ACTION_CHAN)
            if self.chan is None:
                logging.error("Action channel could not be fetched, does it exist/can this bot access it?")
                raise ValueError("Error while finding channel")
        if not isinstance(self.chan, discord.TextChannel):
            logging.error(f"Action channel was not a text channel, it was a {type(chan)} {chan} -- WTF?!")
    ...
    async def on_raw_reaction_remove(self, payload):
        ...
        logging.debug(f"Reaction deleted in my channel: {payload}")
        if self.my_message is None or payload.message_id == self.my_message.id:
            logging.info("Reaction deleted from original message, resetting one-click interaction.")
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        logging.debug(f"Reaction added in my channel: {payload}")
    ...
    async def send_confirmation_message(self):
        ...
        self.current_confirmation_msg = await self.chan.send(f"You are about to give the role {to_assign.mention} to {self.current_confirmation_target.mention}. Is this what you wanted to do?", allowed_mentions=discord.AllowedMentions.none())
    ...
    async def construct_confirmation(self, payload):
        ...
        if not member:
            logging.info("Unable to find the member who reacted.")
            return
        ...
        logging.debug(f"These members are waiting for their interviews and are in the VC: {waiting_targets}")
        if len(waiting_targets) == 1:
            logging.debug("Only one member, that is the target.")
            ...
            logging.debug("Too many possible targets.")
        ...
        logging.debug(f"These members have neither passed nor failed roles: {unassigned_targets}.")
        ...
        logging.debug(f"This is an intention to assign role {to_assign}")
        opposite_polarity_targets = [i for i in members if to_assign not in i.roles]
        logging.debug(f"These members do not have it: {opposite_polarity_targets}")
